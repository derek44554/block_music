...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) => _loadSongs(reset: true));
  }
  ...
  Future<void> _loadSongs({bool reset = false}) async {
    ...
    if (col == null && _colProvider.collections.isEmpty) {
      setState(() { _songs = []; _loading = false; });
      return;
    }
    ...
    if (reset) {
      ...
      if (col != null) {
        ...
        final cachedBids = await service.loadCachedBids(col.bid, tag: _selectedTag);
        if (cachedBids.isNotEmpty) {
          _allBids = cachedBids;
          final songs = await service.getLocalSongs(bids: _allBids, page: 1);
          if (mounted) {
            setState(() { _songs = songs; _page = 2; _hasMore = songs.length == 40; _loading = false; });
  ...
  Future<void> _syncInBackground(MusicService service, String collectionBid, {String? tag}) async {
    if (_syncing) return;
    if (mounted) setState(() => _syncing = true);
    try {
      final bids = await service.syncCollectionBids(collectionBid: collectionBid, tag: tag);
      _allBids = bids;
      final songs = await service.getLocalSongs(bids: bids, page: 1);
      if (mounted) {
        setState(() { _songs = songs; _page = 2; _hasMore = songs.length == 40; });
      }
    } catch (_) {
      // 后台同步失败不影响已展示内容
    } finally {
  ...
  Future<void> _syncAllInBackground(MusicService service) async {    if (_syncing) return;
    if (mounted) setState(() => _syncing = true);
    try {
      final allBids = <String>[];
      for (final c in _colProvider.defaultCollections) {
  ...
  void _play(Song song) {
    ...
    AudioService.instance.setQueue(_songs, _songs.indexWhere((s) => s.bid == song.bid));
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: _buildAppBar(connProvider, hasCollections),
      endDrawer: _MusicDrawer(
      ...
      bottomNavigationBar: _nowPlaying != null
          ? MediaQuery.removePadding(
              context: context,
              removeTop: true,
              child: _MiniPlayerBar(song: _nowPlaying!, onTap: () => _openPlayer(_nowPlaying!)),
            )
          : null,
    );
  }
        ...
        children: [
          Container(
            ...
            decoration: BoxDecoration(
              gradient: const LinearGradient(colors: [AppTheme.primary, Color(0xFF9B59B6)]),
          ...
          if (_syncing) ...[
            ...
            SizedBox(
              ...
              child: CircularProgressIndicator(
                strokeWidth: 2, color: AppTheme.primary.withOpacity(0.7),
              ),
            ),
          ],
        ],
          ...
          child: Column(
            ...
            children: [
              Container(
                ...
                child: const Icon(Icons.cloud_off_rounded, size: 40, color: AppTheme.textSecondary),
              ),
              const SizedBox(height: 24),
              const Text('尚未配置节点',
                  style: TextStyle(color: AppTheme.textPrimary, fontSize: 18, fontWeight: FontWeight.w600)),
              const SizedBox(height: 8),
              const Text('添加一个 Block 节点来开始播放你的音乐',
  ...
  Widget _buildBody(bool hasCollections) {
  ...
  Widget _buildCollectionHeader() {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        gradient: LinearGradient(colors: [c.withOpacity(0.5), AppTheme.card]),
      ),
      child: Row(
        children: [
          Container(
          ...
          child: Column(
            ...
            children: [
              Container(
                width: 80, height: 80,
                decoration: BoxDecoration(color: AppTheme.card, shape: BoxShape.circle),
                child: const Icon(Icons.library_music_outlined, size: 40, color: AppTheme.textSecondary),
              ),
              const SizedBox(height: 24),
              const Text('还没有歌单',
                  style: TextStyle(color: AppTheme.textPrimary, fontSize: 18, fontWeight: FontWeight.w600)),
              const SizedBox(height: 8),
              const Text('点击添加一个集合 BID 来开始播放',
                  textAlign: TextAlign.center,
                  style: TextStyle(color: AppTheme.textSecondary, fontSize: 14)),
              const SizedBox(height: 32),
              FilledButton.icon(
  ...
  Widget _buildEmpty() => const Center(
        child: Padding(
          ...
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              Icon(Icons.music_off_rounded, size: 48, color: AppTheme.textSecondary),
              SizedBox(height: 16),
              Text('该集合暂无歌曲', style: TextStyle(color: AppTheme.textSecondary, fontSize: 15)),
            ],
          ),
        ),
      );
  ...
  void _showAddCollectionDialog(BuildContext context) {
    ...
    showDialog<void>(
      ...
      builder: (_) => AlertDialog(
        ...
        title: const Text('添加集合', style: TextStyle(color: AppTheme.textPrimary)),
        content: TextField(
          controller: ctrl,
          autofocus: true,
          ...
          child: Column(
            ...
            children: [
              ...
              ListTile(
                ...
                title: const Text('选择集合', style: TextStyle(color: AppTheme.textPrimary)),
                onTap: () {
                  Navigator.pop(context);
                    ...
                    child: ListView(
                      ...
                      children: collections.isEmpty
                          ...
                          : collections.map((col) {
                              ...
                              final c = _collectionColor(collections.indexOf(col));
                              return ListTile(
                                contentPadding:
  ...
  Future<void> _deleteSong(Song song) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        title: const Text('删除歌曲', style: TextStyle(color: AppTheme.textPrimary)),
        content: Text('确定要删除「${song.title}」吗？此操作不可撤销。',
        ...
        child: Column(
          ...
          children: [
            ...
            _DrawerTile(
              ...
              label: '全部',
              selected: widget.selectedIndex == null && widget.selectedTag == null,
                    ...
                    return Column(
                      ...
                      children: [
                        _DrawerTile(
                          ...
                          iconColor: col.isDefault ? Colors.amber : c,
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              ListTile(
                leading: const Icon(Icons.delete_outline_rounded, color: Colors.redAccent),
                title: Text('删除「${col.title ?? col.bid.substring(0, 8)}」',
                    style: const TextStyle(color: AppTheme.textPrimary)),
                onTap: () {
                  Navigator.pop(context);
              ...
              ListTile(
                ...
                title: const Text('取消', style: TextStyle(color: AppTheme.textSecondary)),
            ...
            children: [
              Container(
                ...
                child: Icon(icon, size: 18,
                    color: selected ? AppTheme.primary : (iconColor ?? AppTheme.textSecondary)),
              ),
...
class _MiniPlayerBar extends StatefulWidget {
  final Song song;
  final VoidCallback onTap;
  const _MiniPlayerBar({required this.song, required this.onTap});
                      ...
                      children: [
                        ...
                        GestureDetector(
                          ...
                          onHorizontalDragUpdate: (d) {
                            ...
                            setState(() => _seekValue = ratio);
                          },
                          onHorizontalDragEnd: (_) {
                            setState(() => _seeking = false);
                            AudioService.instance.seek(Duration(
                                milliseconds: (_seekValue * total.inMilliseconds).round()));
                          },
                          onTapDown: (d) {
                            final box = context.findRenderObject() as RenderBox?;
                            if (box == null) return;
                            ...
                            children: [
                              ...
                              IconButton(
                                icon: Icon(Icons.skip_next_rounded,
                                    color: AudioService.instance.nextSong !=
                                            null
                                        ? Colors.white
                                        : Colors.white38,
                                    size: 28),
